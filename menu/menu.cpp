...
void projectionUsage(glm::mat4 & projectionMatrix) {
	...
	if (useProjectionGLM) {
		projectionMatrix = glm::perspective(45.0f, 800.f/800.f, 0.001f, 100.0f);
	} else if (useProjectionMyOwn) {
		projectionMatrix = myOwnProjectionMatrix(0.001, 100.0, 45., 800.f/800.f);
	}
} 


void vertexMatrixCalculation(Object3D & object3D, glm::mat4 &proj_matrix, const float & vertex_w) {
	auto model_matrix = object3D.modelMatrix;
	auto vertexes = object3D.geometry.vertex_data;
