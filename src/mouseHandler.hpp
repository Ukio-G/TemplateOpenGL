...
public:
	...
	void pollingMouseEvents() {
		bool leftBtnPressed = glfwGetMouseButton(window->glfwWindow, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS;
		bool rightBtnPressed = glfwGetMouseButton(window->glfwWindow, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS;

		double pos[2] {};
		double delta_pos[2] {};
		glfwGetCursorPos(window->glfwWindow, pos, pos + 1);
	...
	void assignNewEvent(EMouseEvent event, ActionType & action) {
		...
		switch (event) {
			...
			case EME_SCROLL: {
				...
				glfwSetScrollCallback(window->glfwWindow, (callbackType)&callback);
			}
				break;
			case EME_MOVE:
